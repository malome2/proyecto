...
async function sendConfirmacionCambio(to, code, username = '', tipo = 'password') {
    ...
    await transporter.sendMail({
        from: process.env.SMTP_FROM || `Virtual VideoGame Museum <${process.env.SMTP_USER}>`,
        to,
        subject: `${asunto} — Virtual VideoGame Museum`,
        attachments: [{
            filename: 'logo.svg',
            path: LOGO_PATH,
            cid: 'logo@vgmuseum'
    ...
    <table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;border-radius:16px;overflow:hidden;border:1px solid rgba(61,143,181,0.18);">
      ...
      <tr>
        <td style="background:#0a0a18;padding:44px 40px 36px;">
          ...
          <p style="margin:0 0 36px;font-size:0.88rem;color:#8888aa;line-height:1.7;">
            Hemos recibido una solicitud para <strong style="color:#c8c8e8;">${accion}</strong>
            en tu cuenta de <strong style="color:#c8c8e8;">Virtual VideoGame Museum</strong>.
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:8px;">
            <tr>
              <td style="border-left:3px solid rgba(224,92,110,0.5);padding:14px 18px;background:rgba(224,92,110,0.05);border-radius:0 8px 8px 0;">
                <p style="margin:0;font-size:0.82rem;color:#6a6a88;line-height:1.6;">
                  Si no has solicitado este cambio, ignora este correo. Tu cuenta permanece segura.
                </p>
              </td>
            </tr>
          </table>
...
async function sendPasswordResetEmail(to, resetUrl, username = '') {
    await transporter.sendMail({
        from: process.env.SMTP_FROM || `Virtual VideoGame Museum <${process.env.SMTP_USER}>`,
        to,
        subject: 'Recupera tu contraseña — Virtual VideoGame Museum',
        attachments: [{
            filename: 'logo.svg',
            path: LOGO_PATH,
            cid: 'logo@vgmuseum'
        ...
        <td style="background:#0a0a18;padding:44px 40px 36px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
            <tr>
              <td align="center">
                <a href="${resetUrl}"
                   style="display:inline-block;padding:14px 36px;background:#3d8fb5;color:#ffffff;font-size:0.9rem;font-weight:700;text-decoration:none;border-radius:10px;letter-spacing:0.05em;">
          ...
          <p style="margin:0 0 16px;font-size:0.78rem;color:#55556a;line-height:1.6;">
            O copia y pega este enlace en tu navegador:
          </p>
          <p style="margin:0 0 32px;font-size:0.75rem;color:#3d8fb5;word-break:break-all;line-height:1.5;">
            ${resetUrl}
          </p>
